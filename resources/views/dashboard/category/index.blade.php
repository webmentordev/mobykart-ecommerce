<x-app-layout>
    ...
    <div class="py-3">
        <div class="max-w-[99%] mx-auto p-3">
            <form action="{{ route('admin.category.create') }}" method="post" class="flex items-center mb-3">
                @csrf
                @method('POST')
                <div class="w-full">
                    <x-text-input id="title" class="block mt-1 w-full" placeholder="Category name" type="text" name="title" :value="old('title')" required />
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                            ...
                            @foreach ($categories as $item)
                                <tr class="odd:bg-gray-100">
                                    ...
                                    <td class="text-end p-2">
                                        <form action="{{ route('admin.category.status', $item->slug) }}" method="post">
                                            @csrf
                                            @method('PATCH')
                                            @if ($item->is_active)
                    ...
                    @else
                        <p class="text-center py-2 px-4">No category data exist at the moment!</p>
                    @endif
                </div>
            </div>
        </div>
