                                ...
                                <tr class="odd:bg-gray-100">
                                    ...
                                    <td class="text-end p-2 flex justify-end relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" x-cloak x-transition class="fixed bottom-3 right-3 p-3 w-[600px] bg-white shadow-lg rounded-lg border border-gray-200">
                                            ...
                                            <form action="{{ route('admin.brand.update', $item->slug) }}" method="post" class="flex items-center">
                                                @csrf
                                                @method('patch')
                                                <div class="w-full">
                                                    <x-text-input id="title" class="block mt-1 w-full" placeholder="Brand name" type="text" name="title" :value="$item->title" required />
                                                    <x-input-error :messages="$errors->get('title')" class="mt-2" />
                                                </div>
                                                <x-primary-button class="ml-3 py-3">
                                                    {{ __('Update') }}
